	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(async function(){
			...
			var defaults = {
					...
					languageUrl		 : '/control/languages/en.json',
					/* 提供给外部的接口方法 */
			...
			let language = await $.getJSON(para.languageUrl);
			...
			this.replaceAll = function (str, exp, newStr) {
			...
			this.createHtml = function(){
				...
				var html= '';
				console.log("hidden data is: ", para.hiddenData);
				let hiddenData = "";
				if (para.hiddenData) {
					for (let key of Object.keys(para.hiddenData)) {
						console.log("key: ", key, ",val: ", para.hiddenData[key]);
						hiddenData += `<input type="hidden" name="${key}" value="${para.hiddenData[key]}" />`;
					}
				}
				if(para.dragDrop){
					// 创建带有拖动的html
	            	...
	            	html += '						<div class="filePicker">' + language.file_choose_button_name + "</div>";
	            	...
	            	html += '					</div>';
	            	html += '				</div>';
					html += '				<span id="fileDragArea" class="upload_drag_area">'+ language.drag_box_tip +'</span>';
					html += '			</div>';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">'+this.format(language.status_info,{"number":"0","total":"0B"})+'</div>';
		            html += '				<div class="btns">';
		            // html += '					<div class="webuploader_pick">'+ language.continue_choose_button_name +'</div>';
		            html += '					<div class="upload_btn">'+ language.file_upload_button_name +'</div>';
		            html += '				</div>';
		            html += '			</div>';
					html += '			<div id="preview" class="upload_preview"></div>';
					html += '		</div>';
					html += '		<div class="upload_submit">';
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">'+language.confirm_upload_button_name+'</button>';
					html += '		</div>';
		            ...
		            // html += '					<div class="webuploader_pick">'+language.file_choose_button_name+'</div>';
		            html += '					<div class="upload_btn">'+language.file_upload_button_name+'</div>';
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div id="preview" class="upload_preview">';
				    html += '				<div class="add_upload">';
				    html += '					<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" title="'+language.file_choose_tip+'" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
				    ...
				    html += '							<span class="icon iconfont icon-Cloudupload" style="font-size:2em;"></span>';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				$("#status_info").html(this.format(language.status_info,{"number":num,"total":size}));
			};
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 51200000) {
						alert(this.format(language.file_size_exceed_tip, {"filename":file.name}));	
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var delHtml = "";
				if(para.del){  // 显示删除按钮
					delHtml = '<span class="file_del" data-index="'+file.index+'" title="'+language.delete_button_tip+'"></span>';
				}
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure">'+language.upload_failure_tip+'</p>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						// $("#uploadInf").append("<p>" + this.format(language.file_upload_success_tip,{"url":response}) + "</p>");
						$("#uploadInf").append("<p> The image url is " + response + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
					...
					onFailure: function(file) {
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						// $("#uploadInf").append("<p>" + this.format(language.file_upload_failure_tip, file.name) + "</p>");	
						$("#uploadInf").append("<p>Upload failure</p>");	
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length > 0){
						...
						alert(language.empty_file_upload_tip);
					}
	            });
